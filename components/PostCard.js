...
import { ThumbUpIcon, CakeIcon } from "@heroicons/react/solid";
import {
  ChatAltIcon,
  MinusCircleIcon,
  ShareIcon,
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {post.isEvent &&
          <div className="ml-2 mt-5 flex items-center">
            <CakeIcon className="h-11 w-11 text-purple-500"/>
            <div>
              <EventName className="ml-3 leading-2">{post.eventDetails.name}</EventName>
              {post.eventDetails.date && <EventDate className="ml-3">{post.eventDetails.date}</EventDate>}
            </div>
          </div>
        }
